...
for year in years:
    ...
    df_S[df_S < 0] = 0
    df_S.to_csv('Initial_data/BA_solar_{}_init.csv'.format(year))
    
    df_W = pd.DataFrame(W_end, columns=abbr)
    df_W[df_W < 0] = 0
    df_W.to_csv('Initial_data/BA_wind_{}_init.csv'.format(year))
    
    df_L = pd.DataFrame(L_end, columns=abbr)
    df_L[df_L < 0] = 0
    df_L.to_csv('Initial_data/BA_load_{}_init.csv'.format(year))
