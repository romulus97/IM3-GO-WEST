...
for year in years:
    ...
    plants = pd.read_excel('Plant_EIA860_{}.xlsx'.format(year),header=1)
    
    #Reading EIA solar and wind data
    solar = pd.read_excel('Solar_EIA860_{}.xlsx'.format(year),sheet_name='Operable',header=1)
    wind = pd.read_excel('Wind_EIA860_{}.xlsx'.format(year),sheet_name='Operable',header=1)
    ...
    #Exporting results
    solar_wind_df.to_csv('BA_solar_wind_capacity_EIA_{}.csv'.format(year))
