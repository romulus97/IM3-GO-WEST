...
congested_line_scaling = [*range(50,950)]
    ...
    for UC in UC_TREATMENTS:
        ...
        for T_p in line_limit_scaling:
            ...
            for L_s in congested_line_scaling:
                ...
                path=str(Path.cwd()) + str(Path('/Simulation_folders/Exp' + str(NN) + UC + '_' + str(T_p) + '_' + str(L_s)))
                ...
                congested = pd.read_csv('Line_utilization/lines_congested_{}{}_{}.csv'.format(NN,UC,T_p),header=0)
                congested = list(congested['Lines'])
                uncongested = pd.read_csv('Line_utilization/lines_uncongested_{}{}_{}.csv'.format(NN,UC,T_p),header=0)
