# -*- coding: utf-8 -*-
"""
Created on Mon Aug 23 22:05:54 2020
...
BA_solar = pd.read_csv('BA_raw_data/BA_solar_2020_init.csv',header=0)
del BA_solar['Unnamed: 0']

BA_wind = pd.read_csv('BA_raw_data/BA_wind_2020_init.csv',header=0)
...
if missing_value_count_solar > 0:
    ...
    for BA in BAs:
        ...
        if missing_value_number > len(hours_2020)-100:
            BA_solar[BA] = np.repeat(0,len(hours_2020))
            continue
...
solar_BAs = ['AVA','BANC','IID','PACE','PACW','WACM','WALC']
...
if missing_value_count_wind > 0:
    ...
    for BA in BAs:
        ...
        if missing_value_number > len(hours_2020)-100:
            BA_wind[BA] = np.repeat(0,len(hours_2020))
            continue
...
BA_wind.to_csv('BA_organized_data/BA_wind_2020.csv')

BA_solar.drop(feb_29_hours,inplace=True)
BA_solar.reset_index(drop=True,inplace=True)
BA_solar.to_csv('BA_organized_data/BA_solar_2020.csv')           
